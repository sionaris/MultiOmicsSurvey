...
class Monet:
    ...
    def build_a_graph_similarity(self, distances):
        ...
        g = nx.from_numpy_array(res) # original: from_numpy_matrix
        mapping = {}
	...
	"""
        ...
        for omic, dat in data.items():
            ...
            if percentile_remove_edge is not None:
                ...
                # Positive weights percentile
                positive_weights = all_weights_array[all_weights_array > 0] 
                if positive_weights.size > 0:
                    positive_thresh = np.percentile(positive_weights, percentile_remove_edge)
                else:
                    positive_thresh = None  # No positive weights to threshold
            
                # Negative weights percentile
                negative_weights = all_weights_array[all_weights_array < 0]
                if negative_weights.size > 0:
                    negative_thresh = np.percentile(negative_weights, 100 - percentile_remove_edge)
                else:
                    negative_thresh = None  # No negative weights to threshold
                all_edges = []
                for edge in graph.edges:
