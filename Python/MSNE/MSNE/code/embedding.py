...
from code.based import *
from functools import reduce
from code.walker import RandomWalker
...
def MSNE(views, input_type="omic_matrices", n_clusters=5, k=20,workers=4, walk_length=20, num_walks=100, embed_size=100, window_size=10):
    ...
    :param views: 
        - if input_type == "omic_matrices": list of pd.DataFrame, row = sample, col = feature
        - if input_type == "symmetric_relationships": list of symmetrical pd.DataFrame
          (rows = columns = sample IDs)
    :param input_type: "omic_matrices" or "symmetric_relationships"
    ...
    if input_type == "omic_matrices":
        # Original MSNE
        for df in views:
            s = similarity(df, k=k, input_type="omic_matrices")
            # Build networkx graph from the adjacency
            S.append(nx.DiGraph(s))

    elif input_type == "symmetric_relationships":
        # If these are already symmetrical “distance/similarity” matrices
        for s_df in views:
            s = similarity(s_df, k=k, input_type="symmetric_relationships")
            S.append(nx.DiGraph(s_df))

    else:
        raise ValueError("Invalid input_type. Must be 'omic_matrices' or 'symmetric_relationships'")
