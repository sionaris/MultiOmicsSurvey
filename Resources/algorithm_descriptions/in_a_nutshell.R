...
# COCA
desc_list[["COCA"]] = "Cluster Of Clustering Assignments (COCA) is clustering each modality separately either by PAM, hierarchical clustering or $k$-means, determines the best $k$ for each modality based on the silhouette index and then aggregates the results in an indicator matrix for the clusters of all modalities. This matrix is used as input for a final clustering based on the Jaccard index to determine the final number of multi-omic clusters."
