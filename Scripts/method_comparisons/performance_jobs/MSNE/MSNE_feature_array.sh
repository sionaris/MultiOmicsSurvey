...
#SBATCH --array=1-5%2
#SBATCH --job-name=MSNE_feat
#SBATCH --output=logs/MSNE_feat_%A_%a.out
#SBATCH --error=logs/MSNE_feat_%A_%a.err
...
export MSNE_CENTILE="${CENTILES[$((SLURM_ARRAY_TASK_ID-1))]}"
...
echo "[$(date)] ${METHOD} feature task ${SLURM_ARRAY_TASK_ID} centile=${MSNE_CENTILE} starting"

# 1) R: prepare distance matrices
srun --hint=nomultithread --cpu-bind=cores \
  Rscript "${SLURM_SUBMIT_DIR}/Jobs/MSNE/MSNE_feature_experiments.R" prepare \
  > "${OUT_DIR}/prepare.stdout.txt" 2> "${OUT_DIR}/prepare.stderr.txt"

# 2) Python: run MSNE (explicit env python + PYTHONPATH)
...
if [[ ! -d "${MSNE_SRC_DIR}/code" ]]; then
  echo "ERROR: Expected MSNE source dir to contain 'code/' at: ${MSNE_SRC_DIR}" >&2
  exit 3
fi

export PYTHONPATH="${MSNE_SRC_DIR}:${PYTHONPATH:-}"

# Preflight: verify python + numpy on the compute node
...
# 3) R: postprocess
srun --hint=nomultithread --cpu-bind=cores \
  Rscript "${SLURM_SUBMIT_DIR}/Jobs/MSNE/MSNE_feature_experiments.R" post \
