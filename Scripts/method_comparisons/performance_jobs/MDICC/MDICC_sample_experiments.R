...
load_mdicc_stack <- function() {
  ...
  if (!exists("MDICC", mode = "function", inherits = TRUE)) {
    stop("MDICC() missing after sourcing NetworkFusion.R (Resources/MDICC mismatch).")
  }
  if (!exists("testaff", mode = "function", inherits = TRUE)) {
    message("testaff() missing; defining SNFtool-based fallback.")
    fallback_testaff()
  }
  if (!exists("MDICClabel", mode = "function", inherits = TRUE)) {
    message("MDICClabel() missing; defining spectral-clustering fallback.")
    fallback_MDICClabel()
  }
  invisible(TRUE)
}
...
if (!all(need_cols %in% colnames(sample_subsets))) {
  stop("sample_subsets.tsv.gz must contain fraction, replicate, sample_id")
}

mods_required <- c("RNAseq","CNV","Methylation","miRNA","SNPs")
if (!all(mods_required %in% names(input_full))) {
  stop("mm_input missing modalities: ", paste(setdiff(mods_required, names(input_full)), collapse=", "))
}
...
run_mdicc_fixed <- function(X_sf) {
  ...
  list(S = S, label = as.integer(label_vec))
}
...
for (i in seq_len(nrow(pairs))) {
  ...
  pct <- as.integer(round(100 * frac))
  message(sprintf("[%s] %s sample subset %d%% rep %d", Sys.time(), algorithm, pct, repi))
  ...
  clust_file <- file.path(out_root, sprintf("%s_clusters_%dpct_rep%d.tsv.gz", algorithm, pct, repi))
