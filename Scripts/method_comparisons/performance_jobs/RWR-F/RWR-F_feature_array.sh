...
#SBATCH --job-name=RWR-F_feat
#SBATCH --output=logs/RWR-F_feat_%A_%a.out
#SBATCH --error=logs/RWR-F_feat_%A_%a.err
#SBATCH --exclusive
#SBATCH --array=1-5%1

set -euo pipefail

# Keep srun happy on some Slurm installs
export SRUN_CPUS_PER_TASK="${SLURM_CPUS_PER_TASK:-1}"
export SLURM_TRES_PER_TASK="cpu:${SLURM_CPUS_PER_TASK:-1}"

# No parallelisation (even if extra CPUs are requested for memory entitlement)
...
export RWR_FEATURE_CENTILE="${CENTILES[$((SLURM_ARRAY_TASK_ID-1))]}"
...
/usr/bin/time -v -o "${META_DIR}/usrbin_time_v.txt" \
  ...
  Rscript "Jobs/RWR-F/RWR-F_feature_experiments.R" \
    --centile "${RWR_FEATURE_CENTILE}"
rc=$?
set -e
