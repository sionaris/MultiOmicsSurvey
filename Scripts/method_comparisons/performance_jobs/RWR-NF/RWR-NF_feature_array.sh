...
#SBATCH --job-name=RWR-NF_feat
#SBATCH --output=logs/RWR-NF_feat_%A_%a.out
#SBATCH --error=logs/RWR-NF_feat_%A_%a.err
#SBATCH --exclusive
#SBATCH --array=1-5%1
...
export RWRNF_FEATURE_CENTILE="${CENTILES[$((SLURM_ARRAY_TASK_ID-1))]}"
...
/usr/bin/time -v -o "${META_DIR}/usrbin_time_v.txt" \
  ...
  Rscript "Jobs/RWR-NF/RWR-NF_feature_experiments.R" \
    --centile "${RWRNF_FEATURE_CENTILE}"
rc=$?
set -e
