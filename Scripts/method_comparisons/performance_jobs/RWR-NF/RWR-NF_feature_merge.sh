...
#SBATCH --job-name=RWR-NF_feat_merge
#SBATCH --output=logs/RWR-NF_feat_merge_%x_%j.out
#SBATCH --error=logs/RWR-NF_feat_merge_%x_%j.err
...
RUN_DIR="${ROOT}/Results/Performance/Feature_perturbations/RWR-NF/array_${ARRAY_JOB_ID}"
export BENCH_RUN_DIR="$(readlink -f "${RUN_DIR}")"

cd "${ROOT}"

srun --hint=nomultithread --cpu-bind=cores --export=ALL \
  Rscript "Jobs/RWR-NF/RWR-NF_merge.R" --mode feature
