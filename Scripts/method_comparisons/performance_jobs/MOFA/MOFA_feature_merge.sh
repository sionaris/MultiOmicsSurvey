...
#SBATCH --job-name=MOFA_feat_merge
#SBATCH --output=logs/MOFA_feat_merge_%j.out
#SBATCH --error=logs/MOFA_feat_merge_%j.err
...
srun --hint=nomultithread --cpu-bind=cores --export=ALL \
  Rscript --vanilla "${SLURM_SUBMIT_DIR}/Jobs/MOFA/MOFA_merge_perf.R" \
    --run_dir "${RUN_DIR}" \
    --mode feature \
    --out_file "${OUT_DIR}/MOFA_feature_perturbations_performance.tsv" \
  > "${OUT_DIR}/merge.stdout.txt" 2> "${OUT_DIR}/merge.stderr.txt"

echo "MOFA feature merge done: ${OUT_DIR}/MOFA_feature_perturbations_performance.tsv"
