...
#SBATCH --array=1-50%2
#SBATCH --job-name=MOFA_samp
#SBATCH --output=logs/MOFA_samp_%A_%a.out
#SBATCH --error=logs/MOFA_samp_%A_%a.err
...
PREP_META="${META_DIR}/MOFA_prepare_meta_task_${SLURM_ARRAY_TASK_ID}.tsv"
SAMPLE_IDS="${META_DIR}/MOFA_sample_ids_task_${SLURM_ARRAY_TASK_ID}.tsv"
...
# 1) R: export per-view CSVs for this sample subset
srun --hint=nomultithread --cpu-bind=cores --export=ALL \
  Rscript --vanilla "${SLURM_SUBMIT_DIR}/Jobs/MOFA/MOFA_export_inputs_sample.R" \
...
# 2) Python GPU: train MOFA (BENCHMARKED)
