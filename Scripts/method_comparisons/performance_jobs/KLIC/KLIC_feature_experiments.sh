...
#SBATCH --job-name=KLIC_feat
#SBATCH --output=logs/KLIC_feat.out
#SBATCH --error=logs/KLIC_feat.err
#SBATCH --exclusive

set -euo pipefail

METHOD="${METHOD:-KLIC}"
SCRIPT="${SCRIPT:-Jobs/KLIC/KLIC_feature_experiments.R}"
...
# Fair benchmarking: pin BLAS/OpenMP threads
...
# ---- MOSEK env (platform dir, not /bin) ----
...
# libs may be in bin or lib depending on install
LD_ADD=""
...
# License in repo root (FORCE this to avoid inheriting a bad global env var)
...
# License diagnostic (captures the REAL reason if license is invalid / host-locked / server unreachable)
