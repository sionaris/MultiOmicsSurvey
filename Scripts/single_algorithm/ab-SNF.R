...
hclust_pathway_plots_up = plot_pathway_heatmaps(gsea.lists = hclust_output[grepl("up", names(hclust_output))], 
                                                norm.expr = plotdata$RNAseq, 
                                                present_clusters = c("ab-SNF1", "ab-SNF2"),
...
create_MO_heatmap(matrix = aff_CNV, algorithm = algorithm, 
                  need.diag.zero = TRUE, 
                  clust_annot_pheno = clust_annot_pheno %>% dplyr::rename(`ab-SNF` = abSNF),
...
pca_from_sim_matrix(sim_matrix = aff_CNV, algorithm = algorithm, 
                    clust_res = clust_annot_pheno %>% dplyr::select(samID, `ab-SNF` = abSNF),
                    cluster_colors = cluster_colors_heatmap, 
...
clust_annot_pheno_nonas = clust_annot_pheno_nonas %>% dplyr::rename(`ab-SNF` = abSNF)
...
plotdata_bar_sig = clust_annot_pheno_nonas %>% dplyr::select(`ab-SNF`, Race, Histology, 
                                                             `ER status`, `PR status`, `Menopausal status`,
                                                             Stage)
plotdata_bar_sig$`ab-SNF` = factor(plotdata_bar_sig$`ab-SNF`)
...
ggarrange(abSNF_barcharts_sig[[1]], abSNF_barcharts_sig[[2]], abSNF_barcharts_sig[[3]],
          abSNF_barcharts_sig[[4]], abSNF_barcharts_sig[[5]], abSNF_barcharts_sig[[6]],
...
Pheno_sunburst_abSNF$`Menopausal status` = gsub("Indeterminate", "Indet", Pheno_sunburst_abSNF$`Menopausal status`)
Pheno_sunburst_abSNF$`Menopausal status` = gsub("Pre-menopausal", "Pre", Pheno_sunburst_abSNF$`Menopausal status`)
Pheno_sunburst_abSNF$`Menopausal status` = gsub("Perimenopausal", "Peri", Pheno_sunburst_abSNF$`Menopausal status`)
Pheno_sunburst_abSNF$`Menopausal status` = gsub("Post-menopausal", "Post", Pheno_sunburst_abSNF$`Menopausal status`)
Pheno_sunburst_abSNF$`Menopausal status` = gsub("Unknown", "Unkn Meno", Pheno_sunburst_abSNF$`Menopausal status`)
Pheno_sunburst_abSNF$Stage = gsub("Unknown", "Unkn Stage", Pheno_sunburst_abSNF$Stage)
Pheno_sunburst_abSNF = Pheno_sunburst_abSNF %>%
  dplyr::select(abSNF, `ER status`, `Menopausal status`, Stage) %>%
  group_by(abSNF, `ER status`, `Menopausal status`, Stage) %>%
  summarise(Counts = n()) %>%
  as.data.frame()
...
nodes_data <- data.frame(name = V(g)$name) %>%
  inner_join(clust_annot_pheno %>% dplyr::select(samID, `ab-SNF` = abSNF), by = c("name" = "samID"))
nodes_data[[algorithm]] <- as.factor(nodes_data[[algorithm]])
