...
# However ANF prefers features in columns so we transpose the matrices.
...
for (nn in num_neighbors_range) {
  ...
  for (modality in modalities) {
    aff_mat <- ANF::affinity_matrix(as.matrix(input_dists[[modality]]), k = nn,
                              alpha = 1/6, beta = 1/6) # default values
    nn_list[[modality]] <- list(
      num_neighbors = nn,
...
timestamp()
for (i in 1:length(num_neighbors_range)) {
  nn = num_neighbors_range[i]
  sublist = affinity_object[[paste0("NN = ", nn)]]
  iter_matrices = lapply(sublist, `[[`, "affinity_matrix")
  Fusions[[paste0("NN = ", num_neighbors_range[i])]] = ANF::ANF(iter_matrices, 
                                                                K = nn, weight = NULL,
                           type = "two-step", alpha = c(1, 1, 0, 0, 0, 0, 0, 0),
                           verbose = FALSE)
}
timestamp() # ~0.5 min
rm(nn)
...
column_names = colnames(affinity_object[["NN = 10"]][["RNAseq"]][["affinity_matrix"]])
row_names = rownames(affinity_object[["NN = 10"]][["RNAseq"]][["affinity_matrix"]])
...
similarity_results <- compute_matrix_similarity(Fusions)
print(similarity_results)

# Get summary statistics
nn_summary <- data.frame(
  mean = mean(similarity_results$Pearson),      
  median = median(similarity_results$Pearson),    
  sd = sd(similarity_results$Pearson)         
)

# Display the summaries
print(nn_summary)

conclusion = ifelse(nn_summary$mean > 0.8 && nn_summary$median > 0.8,
...
for (i in 1:length(Fusions)) {
  fusion = Fusions[[i]]
  optks = as.data.frame(list(Fusion = names(Fusions)[i], 
...
# }
# 
# contrast_list <- choose_matrix_contrasts(Fusions)
# skewness_kurtosis_list <- choose_matrix_skewness_kurtosis(Fusions)
# 
...
# Spectral clustering for the estimated optimal k by ANF
RNGversion("4.2.2")
set.seed(123)
...
NN_runs = vector("list", length(Fusions))
names(NN_runs) = names(Fusions)
for (i in 1:length(Fusions)) {
  res = spectral_clustering_eig(Fusions[[i]], optk, type = "rw")
  ...
  NN_runs[[names(Fusions)[i]]][["Results"]] = res
  NN_runs[[names(Fusions)[i]]][["Silhouette"]] = sil
  NN_runs[[names(Fusions)[i]]][["Avg. sil. width"]] = avg_width
}
...
conc_NMI = concordanceNetworkNMI_ANF(list(final_affinity_matrix,
                                      affinity_object[[paste0("NN = ", optN)]]$RNAseq$affinity_matrix,
                                      affinity_object[[paste0("NN = ", optN)]]$CNV$affinity_matrix,
                                      affinity_object[[paste0("NN = ", optN)]]$Methylation$affinity_matrix,
                                      affinity_object[[paste0("NN = ", optN)]]$miRNA$affinity_matrix,
                                      affinity_object[[paste0("NN = ", optN)]]$SNPs$affinity_matrix),
                                 C = optk, type = "rw")
...
rank_sum_nas = sapply(ANF_feature_ranks, function(x) sum(is.na(x$NMI_ranks)))
names(rank_sum_nas) = names(ANF_feature_ranks)
...
for (i in 1:length(ANF_feature_ranks)) {
  names(ANF_feature_ranks[[i]]$NMI_scores) = colnames(input[[i]])
}

# Prepare data for plotting
# Combine all modalities into a single data frame
feature_data <- do.call(rbind, lapply(names(ANF_feature_ranks), function(modality) {
  data.frame(
    Feature = paste(modality, seq_along(ANF_feature_ranks[[modality]]$NMI_scores), sep = "_"),
    Modality = modality,
    NMI_Scores = ANF_feature_ranks[[modality]]$NMI_scores
  )
}))
...
var2comp = scheme$var2comp %>%
  ...
  inner_join(ANF_clusters, by = "Sample.ID") %>%
  tibble::column_to_rownames(var = "Sample.ID") %>%
  mutate(ANF = paste0(algorithm, Cluster)) %>%
  dplyr::select(ANF, everything()) %>%
  dplyr::select(-Cluster)
rm(scheme); gc()

# Silhouette
getSilhouette_ggplot(sil      = optimal_ANF$Silhouette,
...
plot_object = list(clust.res = ANF_clusters %>%
...
fga.ANF <- compFGA_optimized(moic.res     = plot_object,
                             segment      = fga_df,
                             iscopynumber = TRUE, 
...
fga.ANF.COSMIC <- compFGA_optimized(moic.res     = plot_object,
                                    segment      = fga_df,
                                    iscopynumber = TRUE, 
                                    test.method  = "nonparametric", # statistical testing method (Wilcoxon with asymptotic approximation. Consider Kruskall Wallis?)
                                    fig.path     = paste0(home, "/Results/single_algorithm/", algorithm),
...
transNEO_var2comp = transNEO_mm_inputs$`Full pheno` %>%
  ...
  inner_join(expr_conc %>% dplyr::select(Donor.ID = samID, ANF = clust_up),
             by = "Donor.ID")
...
clust_annot_pheno = annCol %>% mutate(Sample.ID = rownames(.)) %>%
  ...
  dplyr::rename(ANF = Cluster, samID = "Sample.ID")
...
pca_from_sim_matrix(sim_matrix = aff_CNV, algorithm = algorithm, 
                    clust_res = clust_annot_pheno %>% dplyr::select(samID, ANF),
                    cluster_colors = cluster_colors_heatmap, 
...
for (i in 1:length(voi)) {
  ...
  chifit = chifit[loc, ]
  ANF_barcharts[[i]] = create_annot_barchart(plotdata = plotdata_bar, fill = voi[i],
                                             chifit = chifit,
                                             na.action = "na.omit",
                                             algorithm = algorithm,
                                             barchart_ylim = 680,
...
names(ANF_barcharts) = voi
rm(loc, chifit)

# Multiplot (PNG) - bar charts
library(ggpubr)
ggarrange(ANF_barcharts[[1]], ANF_barcharts[[2]], ANF_barcharts[[3]],
          ANF_barcharts[[4]], ANF_barcharts[[5]], ANF_barcharts[[6]],
          ANF_barcharts[[7]], ANF_barcharts[[8]], ANF_barcharts[[9]],
          ANF_barcharts[[10]], ANF_barcharts[[11]],
...
for (i in 1:length(voi_sig)) {
  ...
  chifit = chifit[loc, ]
  ANF_barcharts_sig[[i]] = create_annot_barchart(plotdata = plotdata_bar_sig, fill = voi_sig[i],
                                                 chifit = chifit,
                                                 na.action = "na.omit",
                                                 algorithm = algorithm,
                                                 barchart_ylim = 680,
...
names(ANF_barcharts_sig) = voi_sig
rm(loc, chifit)
...
# Sunburst plot ###
library(plotly)
Pheno_sunburst_ANF = clust_annot_pheno
...
pie_ANF = plot_ly() %>%
  add_trace(ids = sunburstDF_ANF$ids, labels= sunburstDF_ANF$labels, 
            parents = sunburstDF_ANF$parents, 
            values= sunburstDF_ANF$values, type='sunburst', branchvalues = 'total',
            insidetextorientation='radial', maxdepth = 5,
            marker = list(colors = sunburstDF_ANF$colors)) %>%
  layout(
    grid = list(columns =1, rows = 1),
    margin = list(l = 0, r = 0, b = 0, t = 0)
  )
pie_ANF
rm(Pheno_sunburst_ANF, sunburstDF_ANF, sunburst_coloring_ANF, pie_ANF); gc()
...
for (i in 1:length(list_aff_S)) {
  ...
  nodes_data <- data.frame(name = V(g)$name) %>%
    inner_join(clust_annot_pheno %>% dplyr::select(samID, ANF),
               by = c("name" = "samID"))
  
  # Set ANF as a factor for coloring
  nodes_data[[algorithm]] <- as.factor(nodes_data[[algorithm]])
  V(g)$ANF <- nodes_data[[algorithm]] # modify `$ANF` manually
  
  # Set color based on ANF
  V(g)$color <- fifelse(V(g)$ANF == paste0(algorithm, "1"), "#2EC4B6", "#E71D36")
...
nodes_data <- data.frame(name = V(g)$name) %>%
  inner_join(clust_annot_pheno %>% dplyr::select(samID, ANF), by = c("name" = "samID"))
nodes_data[[algorithm]] <- as.factor(nodes_data[[algorithm]])
V(g)$ANF <- nodes_data[[algorithm]]
