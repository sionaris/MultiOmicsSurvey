...
# Check coca# Check what kind of arrangement the algorithm requires as input 
...
# Setup ###
timestamp()
COCA_moc = buildMOC(data = input, M = 5,
                    maxK = 10, methods = "hclust", 
...
var2comp = scheme$var2comp %>%
  ...
  inner_join(COCA_clusters, by = "Sample.ID") %>%
  tibble::column_to_rownames(var = "Sample.ID") %>%
  mutate(COCA = paste0(algorithm, Cluster)) %>%
  dplyr::select(COCA, everything()) %>%
...
fga.COCA <- compFGA_optimized(moic.res     = plot_object,
                                    segment      = fga_df,
                                    iscopynumber = TRUE, 
...
fga.COCA.COSMIC <- compFGA_optimized(moic.res     = plot_object,
                                           segment      = fga_df,
                                           iscopynumber = TRUE, 
                                           test.method  = "nonparametric", # statistical testing method (Wilcoxon with asymptotic approximation. Consider Kruskall Wallis?)
                                           fig.path     = paste0(home, "/Results/single_algorithm/", algorithm),
...
COCA_clust_res = COCA_clusters %>% dplyr::rename(samID = Sample.ID, COCA = Cluster)
...
for (i in 1:length(voi_sig)) {
  ...
  chifit = chifit[loc, ]
  COCA_barcharts_sig[[i]] = create_annot_barchart(plotdata = plotdata_bar_sig, fill = voi_sig[i],
                                                        chifit = chifit,
                                                        na.action = "na.omit",
                                                        algorithm = algorithm,
                                                        barchart_ylim = 600,
...
names(COCA_barcharts_sig) = voi_sig
rm(loc, chifit)
...
# Compare these COCA results with the COCA output from MOVICS ###
load("Results/MOVICS_baseline/MOVICS_TCGA_RNAseq-CNV-Methylation-miRNA-SNPs_eval_on_transNEO_moic.res.list.rda")
MOVICS_COCA = moic.res.list$COCA$clust.res

ARI_to_MOVICS_COCA = calculate_ari_index(cluster_df1 = MOVICS_COCA %>%
...
NMI_to_MOVICS_COCA = calculate_nmi_index(cluster_df1 = MOVICS_COCA %>%
                                                 dplyr::rename(Sample.ID = samID,
                                                               Cluster = clust),
                                               cluster_df2 = COCA_clusters,
                                               sample_col = "Sample.ID",
...
params = list(algorithm = algorithm, data_source = data_source, data_types = data_types,
              ...
              NMI_to_MOVICS_COCA = NMI_to_MOVICS_COCA, ARI_to_MOVICS_COCA = ARI_to_MOVICS_COCA,
