...
for (i in 1:length(list_aff_S)) {
  ...
  nodes_data <- data.frame(name = V(g)$name) %>%
    inner_join(clust_annot_pheno %>% dplyr::select(samID, MSNE),
               by = c("name" = "samID"))
  
  # Set MSNE as a factor for coloring
  nodes_data[[algorithm]] <- as.factor(nodes_data[[algorithm]])
  V(g)$MSNE <- nodes_data[[algorithm]] # modify `$MSNE` manually
  
  # Set color based on MSNE
  V(g)$color <- fifelse(V(g)$MSNE == paste0(algorithm, "1"), "#2EC4B6", 
                        fifelse(V(g)$MSNE == paste0(algorithm, "2"), "#E71D36",
                                fifelse(V(g)$MSNE == paste0(algorithm, "3"), "#FF9F1C",
                                        fifelse(V(g)$MSNE == paste0(algorithm, "4"), "#BDD5EA", "#FFA5AB"))))
