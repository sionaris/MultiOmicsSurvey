...
# However clusternomics prefers features in columns so we transpose the matrices.
...
# Export input rds
saveRDS(input, "Resources/clusternomics_input.rds")

# Setup ###
# Hyperparameter tuning

# Data distributions
dataDistributions <- c('binary', 'diagNormal', 'diagNormal', 'diagNormal', 'diagNormal')

# Number of global clusters (high) and clusters per modality (lower)
# as per the official vignette
clusterCounts <- list(global = 20, context = c(10, 10, 10, 10, 10))

#-------------------------------------------------------------------------------
# HOWEVER: According to the source code clusterCounts should have length equal #
# to length(datasets) and later on (under the hood), the number of global      #
# clusters is determined as prod(clusterCounts)                                #
# ------------------------------------------------------------------------------

clusterCounts <- list(global = 3**5 + 1, context = c(3, 3, 3, 3, 3),
                      add1 = 0, add2 = 0, add3 = 0)

# The extra slots are added to avoid errors (BUT I STILL GET THE ERRORS)

# MCMC hyperparameters suggested for real world data
maxIter <- 5000 
burnin <- 2000 
lag <- 10 
...
# Run the algorithm
timestamp()
results <- contextCluster(datasets = input, 
...
for (nn in num_neighbors_range) {
  similarity_object[[paste0("NN = ", nn)]] = clusternomics_mod(X = input, c = ground_truth_k, 
...
clusternomics_clusters = as.data.frame(list(Sample.ID = rownames(consensus_km[["realdataresults"]][[2]][["ordered_annotation"]]),
                                            Cluster = consensus_km[["realdataresults"]][[2]][["ordered_annotation"]][["consensuscluster"]]))
clusternomics_clusters$Sample.ID = gsub("\\.", "-", clusternomics_clusters$Sample.ID)
rownames(clusternomics_clusters) = clusternomics_clusters$Sample.ID
...
ARI_to_MOVICS = calculate_ari_index(cluster_df1 = ground_truth_labels,
                                    cluster_df2 = clusternomics_clusters,
...
var2comp = scheme$var2comp %>%
  ...
  inner_join(clusternomics_clusters, by = "Sample.ID") %>%
  tibble::column_to_rownames(var = "Sample.ID") %>%
  mutate(clusternomics = paste0(algorithm, Cluster)) %>%
  dplyr::select(clusternomics, everything()) %>%
  dplyr::select(-Cluster)
rm(scheme); gc()

# Silhouette
library(MOVICS)
clusternomics_matrix = similarity_object[[paste0("NN = ", optNN)]][["S"]]
dimnames(clusternomics_matrix) = list(colnames(input$SNPs), colnames(input$SNPs))
sil = compute_silhouette(cluster_df = clusternomics_clusters %>% dplyr::rename(samID = Sample.ID),
                         similarity_matrix = clusternomics_matrix,
...
plot_object = list(clust.res = clusternomics_clusters %>%
...
hclust_pathway_plots_up = plot_pathway_heatmaps(gsea.lists = hclust_output[grepl("up", names(hclust_output))], 
                                                norm.expr = plotdata$RNAseq, 
                                                present_clusters = c("clusternomics1", "clusternomics2"),
...
transNEO_var2comp = transNEO_mm_inputs$`Full pheno` %>%
  ...
  inner_join(expr_conc %>% dplyr::select(Donor.ID = samID, clusternomics = clust_up),
             by = "Donor.ID")
...
clust_annot_pheno = annCol %>% mutate(Sample.ID = rownames(.)) %>%
  ...
  dplyr::rename(clusternomics = Cluster, samID = "Sample.ID")
...
clusternomics_clust_res = clusternomics_clusters %>% dplyr::rename(samID = Sample.ID, clusternomics = Cluster)

# PCA from original matrices ###
# RNA
pca_from_original_matrix(mydata = plotdata$RNAseq, 
                         algorithm = algorithm, 
                         clust_res = clusternomics_clust_res,
...
create_MO_heatmap(matrix = clusternomics_matrix, algorithm = algorithm, 
                  need.diag.zero = FALSE, # already zero
                  clust_annot_pheno = clust_annot_pheno ,
                  afh_colnames = afh_colnames, 
                  colors = colors_heatmap,
                  annColors = annColors,
                  heatmap_title = "Final clusternomics similarity (S matrix) heatmap",
                  ...
                  output_file_name = paste0(home, "/Results/single_algorithm/", algorithm, 
                                            "/Supplement/clusternomics_final_S_matrix_heatmap.png"))
...
for (i in 1:length(voi)) {
  ...
  chifit = chifit[loc, ]
  clusternomics_barcharts[[i]] = create_annot_barchart(plotdata = plotdata_bar, fill = voi[i],
                                                       chifit = chifit,
                                                       na.action = "na.omit",
                                                       algorithm = algorithm,
                                                       barchart_ylim = 650,
  ...
  print(clusternomics_barcharts[[i]])
...
names(clusternomics_barcharts) = voi
rm(loc, chifit)

# Multiplot (PNG) - bar charts
library(ggpubr)
ggarrange(clusternomics_barcharts[[1]], clusternomics_barcharts[[2]], clusternomics_barcharts[[3]],
          clusternomics_barcharts[[4]], clusternomics_barcharts[[5]], clusternomics_barcharts[[6]],
          clusternomics_barcharts[[7]], clusternomics_barcharts[[8]], clusternomics_barcharts[[9]],
          clusternomics_barcharts[[10]], clusternomics_barcharts[[11]],
...
clusternomics_barcharts_sig = list()
plotdata_bar_sig = clust_annot_pheno_nonas %>% dplyr::select(clusternomics, Histology, 
                                                             `ER status`, `PR status`)
plotdata_bar_sig$clusternomics = factor(plotdata_bar_sig$clusternomics)
voi_sig = setdiff(colnames(plotdata_bar_sig), algorithm)
for (i in 1:length(voi_sig)) {
  chifit = output
  loc = which(grepl(voi_sig[i], chifit$Comparison))
  chifit = chifit[loc, ]
  clusternomics_barcharts_sig[[i]] = create_annot_barchart(plotdata = plotdata_bar_sig, fill = voi_sig[i],
                                                           chifit = chifit,
                                                           na.action = "na.omit",
                                                           algorithm = algorithm,
                                                           barchart_ylim = 650,
  ...
  print(clusternomics_barcharts_sig[[i]])
...
names(clusternomics_barcharts_sig) = voi_sig
rm(loc, chifit)

# Multiplot (PNG) - bar charts
ggarrange(clusternomics_barcharts_sig[[1]], clusternomics_barcharts_sig[[2]], clusternomics_barcharts_sig[[3]],
...
# Sunburst plot ###
library(plotly)
Pheno_sunburst_clusternomics = clust_annot_pheno
Pheno_sunburst_clusternomics$`ER status` = gsub("Unknown", "Unkn ER status", Pheno_sunburst_clusternomics$`ER status`)
Pheno_sunburst_clusternomics$`ER status` = gsub("Positive", "ER+", Pheno_sunburst_clusternomics$`ER status`)
Pheno_sunburst_clusternomics$`ER status` = gsub("Negative", "ER-", Pheno_sunburst_clusternomics$`ER status`)
Pheno_sunburst_clusternomics = Pheno_sunburst_clusternomics %>%
  dplyr::select(clusternomics, `ER status`) %>%
  group_by(clusternomics, `ER status`) %>%
  summarise(Counts = n()) %>%
  as.data.frame()

sunburst_coloring_clusternomics = data.frame(stringsAsFactors = FALSE,
                                             colors = tolower(gplots::col2hex(c("#2EC4B6", "#E71D36", 
                                                                                "#C11D9C", "#0F1682",  "grey40"))),
                                             labels = c("clusternomics1", "clusternomics2",
                                                        "ER-", "ER+", "Unkn ER status"))

sunburstDF_clusternomics = as.sunburstDF(Pheno_sunburst_clusternomics, value_column = "Counts", add_root = FALSE) %>%
  inner_join(sunburst_coloring_clusternomics, by = "labels")

pie_clusternomics = plot_ly() %>%
  add_trace(ids = sunburstDF_clusternomics$ids, labels= sunburstDF_clusternomics$labels, 
            parents = sunburstDF_clusternomics$parents, 
            values= sunburstDF_clusternomics$values, type='sunburst', branchvalues = 'total',
            insidetextorientation='radial', maxdepth = 5,
            marker = list(colors = sunburstDF_clusternomics$colors)) %>%
  layout(
    grid = list(columns =1, rows = 1),
    margin = list(l = 0, r = 0, b = 0, t = 0)
  )
pie_clusternomics
rm(Pheno_sunburst_clusternomics, sunburstDF_clusternomics, sunburst_coloring_clusternomics, pie_clusternomics); gc()

# Compare these clusternomics results with the clusternomics output from MOVICS ###
load("Results/MOVICS_baseline/MOVICS_TCGA_RNAseq-CNV-Methylation-miRNA-SNPs_eval_on_transNEO_moic.res.list.rda")
MOVICS_clusternomics = moic.res.list$clusternomics$clust.res

ARI_to_MOVICS_clusternomics = calculate_ari_index(cluster_df1 = MOVICS_clusternomics %>%
...
NMI_to_MOVICS_clusternomics = calculate_nmi_index(cluster_df1 = MOVICS_clusternomics %>%
                                                    dplyr::rename(Sample.ID = samID,
                                                                  Cluster = clust),
                                                  cluster_df2 = clusternomics_clusters,
                                                  sample_col = "Sample.ID",
...
params = list(algorithm = algorithm, data_source = data_source, data_types = data_types,
              ...
              NMI_to_MOVICS_clusternomics = NMI_to_MOVICS_clusternomics, ARI_to_MOVICS_clusternomics = ARI_to_MOVICS_clusternomics,
