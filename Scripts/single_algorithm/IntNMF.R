...
# However IntNMF prefers features in columns so we transpose the matrices.
...
if (foreach::getDoParRegistered()) {
  message("Parallel backend registered with ", foreach::getDoParWorkers(), " workers.")
} else {
...
for (i in 1:nrow(tuning_grid)) {
  message("Running for n.runs = ", tuning_grid$n.runs[i], 
          ", maxiter = ", tuning_grid$maxiter[i])
  
  # Call the function
  IntNMF_optk_list[[paste0("n.runs = ", tuning_grid$n.runs[i], 
                           ", maxiter = ", tuning_grid$maxiter[i])]] <- integrative_nmf_plus(
                             dat = input_nn,
                             n.runs = tuning_grid$n.runs[i],
                             maxiter = tuning_grid$maxiter[i],
                             n.fold = 10,
                             kmax = 10,
                             is.binary = c(TRUE, FALSE, FALSE, FALSE, FALSE), # Set binary flags for modalities
                             allowParallel = TRUE,
                             n.cores = n.cores,
                             verbose = TRUE
                           )
  
  cat("Done with n.runs =", tuning_grid$n.runs[i], "/ maxiter =", tuning_grid$maxiter[i], "\n")
}
...
Fusions <- foreach(nn = num_neighbors_range, .combine = 'c', .packages = 'IntNMFtool') %:%
  foreach(sigma = sigma_range, .combine = 'c') %dopar% {
    ...
    fusion_result <- IntNMF(iter_matrices, K = nn, t = n_iterations, parallel = FALSE)
    list(fusion_result)
  }
...
# Spectral clustering for the estimated optimal k by IntNMFtool
RNGversion("4.2.2")
set.seed(123)

final_affinity_matrix = Fusions[[paste0("NN = ", optN, ", sigma = ", optSigma)]]
IntNMF_optks = estimateNumberOfClustersGivenGraph(final_affinity_matrix, NUMC = 2:10)
paste0("The optimal value for k using the eigen-gap method is ",
       IntNMF_optks[[1]], ". The optimal value for k using the rotation method is ",
       IntNMF_optks[[3]], ".")
optk = IntNMF_optks[[1]]
...
IntNMF_clusters = as.data.frame(list(Sample.ID = names(group),
                                  Cluster = group))

# Feature ranking
IntNMF_feature_ranks = list()
binary_flags = c(TRUE, FALSE, FALSE, FALSE, FALSE)
for (i in 1:length(input)) {
  IntNMF_feature_ranks[[i]] = rankFeaturesByNMI_parallely(data = list(input[[i]]), 
                                                       W = final_affinity_matrix,
                                                       ncores = 8,
                                                       binary = binary_flags[i])
  cat("Done with", names(input)[i], "\n")
}
names(IntNMF_feature_ranks) = names(input)

rank_sum_nas = sapply(IntNMF_feature_ranks, function(x) sum(is.na(x$NMI_ranks)))
names(rank_sum_nas) = names(IntNMF_feature_ranks)
...
for (i in 1:length(IntNMF_feature_ranks)) {
  names(IntNMF_feature_ranks[[i]]$NMI_scores) = colnames(input[[i]])
}

# Prepare data for plotting
# Combine all modalities into a single data frame
feature_data <- do.call(rbind, lapply(names(IntNMF_feature_ranks), function(modality) {
  data.frame(
    Feature = paste(modality, seq_along(IntNMF_feature_ranks[[modality]]$NMI_scores), sep = "_"),
    Modality = modality,
    NMI_Scores = IntNMF_feature_ranks[[modality]]$NMI_scores
  )
}))
...
var2comp = scheme$var2comp %>%
  ...
  inner_join(IntNMF_clusters, by = "Sample.ID") %>%
  tibble::column_to_rownames(var = "Sample.ID") %>%
  mutate(IntNMF = paste0(algorithm, Cluster)) %>%
  dplyr::select(IntNMF, everything()) %>%
...
sil = compute_silhouette(cluster_df = IntNMF_clusters %>% dplyr::rename(samID = Sample.ID),
...
plot_object = list(clust.res = IntNMF_clusters %>%
...
hclust_pathway_plots_up = plot_pathway_heatmaps(gsea.lists = hclust_output[grepl("up", names(hclust_output))], 
                                                norm.expr = plotdata$RNAseq, 
                                                present_clusters = c("IntNMF1", "IntNMF2"),
...
fga.IntNMF <- compFGA_optimized(moic.res     = plot_object,
                             segment      = fga_df,
                             iscopynumber = TRUE, 
...
fga.IntNMF.COSMIC <- compFGA_optimized(moic.res     = plot_object,
                                    segment      = fga_df,
                                    iscopynumber = TRUE, 
                                    test.method  = "nonparametric", # statistical testing method (Wilcoxon with asymptotic approximation. Consider Kruskall Wallis?)
                                    fig.path     = paste0(home, "/Results/single_algorithm/", algorithm),
...
transNEO_var2comp = transNEO_mm_inputs$`Full pheno` %>%
  ...
  inner_join(expr_conc %>% dplyr::select(Donor.ID = samID, IntNMF = clust_up),
             by = "Donor.ID")
...
clust_annot_pheno = annCol %>% mutate(Sample.ID = rownames(.)) %>%
  ...
  dplyr::rename(IntNMF = Cluster, samID = "Sample.ID")
...
aff_CNV = normalize_affinity_matrix(
  IntNMFtool::affinityMatrix(
    IntNMFtool::dist2(input$CNV,
                   input$CNV),
...
aff_Methyl = normalize_affinity_matrix(
  IntNMFtool::affinityMatrix(
    IntNMFtool::dist2(input$Methylation,
                   input$Methylation),
    K = optN, sigma = optSigma))
...
pca_from_sim_matrix(sim_matrix = aff_CNV, algorithm = algorithm, 
                    clust_res = clust_annot_pheno %>% dplyr::select(samID, IntNMF),
                    cluster_colors = cluster_colors_heatmap, 
...
plotdata_bar_sig = clust_annot_pheno_nonas %>% dplyr::select(IntNMF, Race, Histology, 
                                                             `ER status`, `PR status`)
...
Pheno_sunburst_IntNMF = Pheno_sunburst_IntNMF %>%
  dplyr::select(IntNMF, `ER status`) %>%
  group_by(IntNMF, `ER status`) %>%
  summarise(Counts = n()) %>%
  as.data.frame()
...
for (i in 1:length(list_aff_S)) {
  ...
  nodes_data <- data.frame(name = V(g)$name) %>%
    inner_join(clust_annot_pheno %>% dplyr::select(samID, IntNMF),
               by = c("name" = "samID"))
  
  # Set IntNMF as a factor for coloring
  nodes_data[[algorithm]] <- as.factor(nodes_data[[algorithm]])
  V(g)$IntNMF <- nodes_data[[algorithm]] # modify `$IntNMF` manually
  
  # Set color based on IntNMF
  V(g)$color <- fifelse(V(g)$IntNMF == paste0(algorithm, "1"), "#2EC4B6", "#E71D36")
...
nodes_data <- data.frame(name = V(g)$name) %>%
  inner_join(clust_annot_pheno %>% dplyr::select(samID, IntNMF), by = c("name" = "samID"))
nodes_data[[algorithm]] <- as.factor(nodes_data[[algorithm]])
V(g)$IntNMF <- nodes_data[[algorithm]]
...
# Compare these IntNMF results with the IntNMF output from MOVICS ###
load("Results/MOVICS_baseline/MOVICS_TCGA_RNAseq-CNV-Methylation-miRNA-SNPs_eval_on_transNEO_moic.res.list.rda")
MOVICS_IntNMF = moic.res.list$IntNMF$clust.res

ARI_to_MOVICS_IntNMF = calculate_ari_index(cluster_df1 = MOVICS_IntNMF %>%
...
NMI_to_MOVICS_IntNMF = calculate_nmi_index(cluster_df1 = MOVICS_IntNMF %>%
                                          dplyr::rename(Sample.ID = samID,
                                                        Cluster = clust),
                                        cluster_df2 = IntNMF_clusters,
                                        sample_col = "Sample.ID",
...
params = list(algorithm = algorithm, data_source = data_source, data_types = data_types,
              ...
              NMI_to_MOVICS_IntNMF = NMI_to_MOVICS_IntNMF, ARI_to_MOVICS_IntNMF = ARI_to_MOVICS_IntNMF,
