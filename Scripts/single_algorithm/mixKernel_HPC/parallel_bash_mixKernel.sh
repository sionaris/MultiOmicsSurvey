...
#SBATCH --nodes=1
#SBATCH --job-name=parallel_mixKernel
#SBATCH --output=logs/parallel_mixKernel.out
#SBATCH --error=logs/parallel_mixKernel.err
...
# Optional: install any missing packages (CRAN)
Rscript -e "packages <- c('parallel', 'foreach'); to_install <- setdiff(packages, installed.packages()[,'Package']); if(length(to_install)>0) install.packages(to_install)"
