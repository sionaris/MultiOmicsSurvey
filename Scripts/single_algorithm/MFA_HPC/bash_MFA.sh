...
#SBATCH --nodes=1
#SBATCH --job-name=MFA
#SBATCH --output=logs/MFA.out
#SBATCH --error=logs/MFA.err
...
Rscript -e "packages <- c('FactoMineR'); to_install <- setdiff(packages, installed.packages()[,'Package']); if(length(to_install)>0) install.packages(to_install)"
