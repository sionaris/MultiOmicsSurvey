...
RWRF_clusters = as.data.frame(list(Cluster = best_clustering$cluster, 
                                   Sample.ID = colnames(Fusions_filt[[paste0("NN = ", optNN, ", sigma = ", optSigma)]])))

RWRF_clusters$Sample.ID = gsub("\\.", "-", RWRF_clusters$Sample.ID)
rownames(RWRF_clusters) = RWRF_clusters$Sample.ID

# Feature ranking
final_affinity_matrix = Fusions_filt[[paste0("NN = ", optN, ", sigma = ", optSigma)]]
RWRF_feature_ranks = list()
binary_flags = c(TRUE, FALSE, FALSE, FALSE, FALSE)
for (i in 1:length(input)) {
  RWRF_feature_ranks[[i]] = rankFeaturesByNMI_parallely(data = list(input[[i]]), 
...
names(RWRF_feature_ranks) = names(input)

rank_sum_nas = sapply(RWRF_feature_ranks, function(x) sum(is.na(x$NMI_ranks)))
names(rank_sum_nas) = names(RWRF_feature_ranks)
...
for (i in 1:length(RWRF_feature_ranks)) {
  names(RWRF_feature_ranks[[i]]$NMI_scores) = colnames(input[[i]])
}

# Prepare data for plotting
# Combine all modalities into a single data frame
feature_data <- do.call(rbind, lapply(names(RWRF_feature_ranks), function(modality) {
  data.frame(
    Feature = paste(modality, seq_along(RWRF_feature_ranks[[modality]]$NMI_scores), sep = "_"),
    Modality = modality,
    NMI_Scores = RWRF_feature_ranks[[modality]]$NMI_scores
  )
}))
...
# Moderate statistics when compared to the MOVICS. Results somewhat similar
...
var2comp = scheme$var2comp %>%
  ...
  inner_join(RWRF_clusters, by = "Sample.ID") %>%
  tibble::column_to_rownames(var = "Sample.ID") %>%
  mutate(RWRF = paste0(algorithm, Cluster)) %>%
  dplyr::select(RWRF, everything()) %>%
...
plot_object = list(clust.res = RWRF_clusters %>%
...
hclust_pathway_plots_up = plot_pathway_heatmaps(gsea.lists = hclust_output[grepl("up", names(hclust_output))], 
                                                norm.expr = plotdata$RNAseq, 
                                                present_clusters = c("RWR-F1", "RWR-F2"),
...
fga.RWRF <- compFGA_optimized(moic.res     = plot_object,
                              segment      = fga_df,
                              iscopynumber = TRUE, 
...
fga.RWRF.COSMIC <- compFGA_optimized(moic.res     = plot_object,
                                     segment      = fga_df,
                                     iscopynumber = TRUE, 
                                     test.method  = "nonparametric", # statistical testing method (Wilcoxon with asymptotic approximation. Consider Kruskall Wallis?)
                                     fig.path     = paste0(home, "/Results/single_algorithm/", algorithm),
...
transNEO_var2comp = transNEO_mm_inputs$`Full pheno` %>%
  ...
  inner_join(expr_conc %>% dplyr::select(Donor.ID = samID, RWRF = clust_up),
             by = "Donor.ID")
...
clust_annot_pheno = annCol %>% mutate(Sample.ID = rownames(.)) %>%
  ...
  dplyr::rename(RWRF = Cluster, samID = "Sample.ID")
...
create_MO_heatmap(matrix = aff_CNV, algorithm = algorithm, 
                  need.diag.zero = TRUE, 
                  clust_annot_pheno = clust_annot_pheno %>% dplyr::rename(`RWR-F` = RWRF),
...
pca_from_sim_matrix(sim_matrix = aff_CNV, algorithm = algorithm, 
                    clust_res = clust_annot_pheno %>% dplyr::select(samID, `RWR-F` = RWRF),
                    cluster_colors = cluster_colors_heatmap, 
...
clust_annot_pheno_nonas = clust_annot_pheno_nonas %>% dplyr::rename(`RWR-F` = RWRF)
...
for (i in 1:length(voi)) {
  ...
  chifit = chifit[loc, ]
  RWRF_barcharts[[i]] = create_annot_barchart(plotdata = plotdata_bar, fill = voi[i],
                                              chifit = chifit,
                                              na.action = "na.omit",
                                              algorithm = algorithm,
                                              barchart_ylim = 650,
...
names(RWRF_barcharts) = voi
rm(loc, chifit)

# Multiplot (PNG) - bar charts
library(ggpubr)
ggarrange(RWRF_barcharts[[1]], RWRF_barcharts[[2]], RWRF_barcharts[[3]],
          RWRF_barcharts[[4]], RWRF_barcharts[[5]], RWRF_barcharts[[6]],
          RWRF_barcharts[[7]], RWRF_barcharts[[8]], RWRF_barcharts[[9]],
          RWRF_barcharts[[10]], RWRF_barcharts[[11]],
...
RWRF_barcharts_sig = list()
plotdata_bar_sig = clust_annot_pheno_nonas %>% dplyr::select(`RWR-F`, Race, Histology, 
                                                             `ER status`, `PR status`)
plotdata_bar_sig$`RWR-F` = factor(plotdata_bar_sig$`RWR-F`)
voi_sig = setdiff(colnames(plotdata_bar_sig), algorithm)
for (i in 1:length(voi_sig)) {
  chifit = output
  loc = which(grepl(voi_sig[i], chifit$Comparison))
  chifit = chifit[loc, ]
  RWRF_barcharts_sig[[i]] = create_annot_barchart(plotdata = plotdata_bar_sig, fill = voi_sig[i],
                                                  chifit = chifit,
                                                  na.action = "na.omit",
                                                  algorithm = algorithm,
                                                  barchart_ylim = 650,
...
names(RWRF_barcharts_sig) = voi_sig
rm(loc, chifit)

# Multiplot (PNG) - bar charts
ggarrange(RWRF_barcharts_sig[[1]], RWRF_barcharts_sig[[2]], RWRF_barcharts_sig[[3]],
          RWRF_barcharts_sig[[4]],
...
# Sunburst plot ###
library(plotly)
Pheno_sunburst_RWRF = clust_annot_pheno
Pheno_sunburst_RWRF$`ER status` = gsub("Unknown", "Unkn ER status", Pheno_sunburst_RWRF$`ER status`)
Pheno_sunburst_RWRF$`ER status` = gsub("Positive", "ER+", Pheno_sunburst_RWRF$`ER status`)
Pheno_sunburst_RWRF$`ER status` = gsub("Negative", "ER-", Pheno_sunburst_RWRF$`ER status`)
Pheno_sunburst_RWRF$`HER2 status` = gsub("Unknown", "Unkn HER2 status", 
                                          Pheno_sunburst_RWRF$`HER2 status`)
Pheno_sunburst_RWRF$`HER2 status` = gsub("Positive", "HER2+", Pheno_sunburst_RWRF$`HER2 status`)
Pheno_sunburst_RWRF$`HER2 status` = gsub("Negative", "HER2-", Pheno_sunburst_RWRF$`HER2 status`)
Pheno_sunburst_RWRF = Pheno_sunburst_RWRF %>%
  dplyr::select(RWRF, `ER status`, `HER2 status`) %>%
  group_by(RWRF, `ER status`, `HER2 status`) %>%
  summarise(Counts = n()) %>%
  as.data.frame()
sunburst_coloring_RWRF = data.frame(stringsAsFactors = FALSE,
                                     colors = tolower(gplots::col2hex(c("#2EC4B6", "#E71D36", 
                                                                        "#C11D9C", "#0F1682",  "grey40",
                                                                        "#0B9EF8", "#560DA7", "mistyrose1", 
                                                                        "hotpink4", "grey40"))),
                                     labels = c("RWR-F1", "RWR-F2",
                                                "ER-", "ER+", "Unkn ER status",
                                                "HER2-", "HER2+", "Indeterminate",
                                                "Equivocal", "Unkn HER2 status"))


sunburstDF_RWRF = as.sunburstDF(Pheno_sunburst_RWRF, value_column = "Counts", add_root = FALSE) %>%
  inner_join(sunburst_coloring_RWRF, by = "labels")

pie_RWRF = plot_ly() %>%
  add_trace(ids = sunburstDF_RWRF$ids, labels= sunburstDF_RWRF$labels, 
            parents = sunburstDF_RWRF$parents, 
            values= sunburstDF_RWRF$values, type='sunburst', branchvalues = 'total',
            insidetextorientation='radial', maxdepth = 5,
            marker = list(colors = sunburstDF_RWRF$colors)) %>%
  layout(
    grid = list(columns =1, rows = 1),
    margin = list(l = 0, r = 0, b = 0, t = 0)
  )
pie_RWRF
rm(Pheno_sunburst_RWRF, sunburstDF_RWRF, sunburst_coloring_RWRF, pie_RWRF); gc()
...
for (i in 1:length(list_aff_S)) {
  ...
  nodes_data <- data.frame(name = V(g)$name) %>%
    inner_join(clust_annot_pheno %>% dplyr::select(samID, `RWR-F` = RWRF),
               by = c("name" = "samID"))
  
  # Set RWRF as a factor for coloring
  nodes_data[[algorithm]] <- as.factor(nodes_data[[algorithm]])
  V(g)$`RWR-F` <- nodes_data[[algorithm]] # modify `$RWR-F` manually
  
  # Set color based on RWRF
  V(g)$color <- fifelse(V(g)$`RWR-F` == paste0(algorithm, "1"), "#2EC4B6", "#E71D36")
...
nodes_data <- data.frame(name = V(g)$name) %>%
  inner_join(clust_annot_pheno %>% dplyr::select(samID, `RWR-F` = RWRF), by = c("name" = "samID"))
nodes_data[[algorithm]] <- as.factor(nodes_data[[algorithm]])
V(g)$`RWR-F` <- nodes_data[[algorithm]]
