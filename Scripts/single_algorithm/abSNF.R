...
# Spectral clustering for the estimated optimal k by abSNF
RNGversion("4.2.2")
set.seed(123)

final_affinity_matrix = Fusions[[paste0("NN = ", optN, ", sigma = ", optSigma)]]
abSNF_optks = estimateNumberOfClustersGivenGraph(final_affinity_matrix, NUMC = 2:10)
paste0("The optimal value for k using the eigen-gap method is ",
       abSNF_optks[[1]], ". The optimal value for k using the rotation method is ",
       abSNF_optks[[3]], ".")
optk = abSNF_optks[[1]]
...
abSNF_clusters = as.data.frame(list(Sample.ID = names(group),
                                  Cluster = group))
...
sil = compute_silhouette(cluster_df = abSNF_clusters %>% dplyr::rename(samID = Sample.ID),
...
hclust_pathway_plots_up = plot_pathway_heatmaps(gsea.lists = hclust_output[grepl("up", names(hclust_output))], 
                                                norm.expr = plotdata$RNAseq, 
                                                present_clusters = c("abSNF1", "abSNF2"),
...
fga.abSNF <- compFGA_optimized(moic.res     = plot_object,
                             segment      = fga_df,
                             iscopynumber = TRUE, 
...
fga.abSNF.COSMIC <- compFGA_optimized(moic.res     = plot_object,
                                    segment      = fga_df,
                                    iscopynumber = TRUE, 
                                    test.method  = "nonparametric", # statistical testing method (Wilcoxon with asymptotic approximation. Consider Kruskall Wallis?)
                                    fig.path     = paste0(home, "/Results/single_algorithm/", algorithm),
...
transNEO_var2comp = transNEO_mm_inputs$`Full pheno` %>%
  ...
  inner_join(expr_conc %>% dplyr::select(Donor.ID = samID, abSNF = clust_up),
             by = "Donor.ID")
...
clust_annot_pheno = annCol %>% mutate(Sample.ID = rownames(.)) %>%
  ...
  dplyr::rename(abSNF = Cluster, samID = "Sample.ID")
...
aff_CNV = normalize_affinity_matrix(
  abSNF::affinityMatrix(
    abSNF::dist2(input$CNV,
                   input$CNV),
...
pca_from_sim_matrix(sim_matrix = aff_CNV, algorithm = algorithm, 
                    clust_res = clust_annot_pheno %>% dplyr::select(samID, abSNF),
                    cluster_colors = cluster_colors_heatmap, 
...
for (i in 1:length(voi)) {
  ...
  chifit = chifit[loc, ]
  abSNF_barcharts[[i]] = create_annot_barchart(plotdata = plotdata_bar, fill = voi[i],
                                             chifit = chifit,
                                             na.action = "na.omit",
                                             algorithm = algorithm,
                                             barchart_ylim = 650,
...
names(abSNF_barcharts) = voi
rm(loc, chifit)

# Multiplot (PNG) - bar charts
library(ggpubr)
ggarrange(abSNF_barcharts[[1]], abSNF_barcharts[[2]], abSNF_barcharts[[3]],
          abSNF_barcharts[[4]], abSNF_barcharts[[5]], abSNF_barcharts[[6]],
          abSNF_barcharts[[7]], abSNF_barcharts[[8]], abSNF_barcharts[[9]],
          abSNF_barcharts[[10]], abSNF_barcharts[[11]],
...
abSNF_barcharts_sig = list()
plotdata_bar_sig = clust_annot_pheno_nonas %>% dplyr::select(abSNF, Race, Histology, 
                                                             `ER status`, `PR status`)
plotdata_bar_sig$abSNF = factor(plotdata_bar_sig$abSNF)
voi_sig = setdiff(colnames(plotdata_bar_sig), algorithm)
for (i in 1:length(voi_sig)) {
  chifit = output
  loc = which(grepl(voi_sig[i], chifit$Comparison))
  chifit = chifit[loc, ]
  abSNF_barcharts_sig[[i]] = create_annot_barchart(plotdata = plotdata_bar_sig, fill = voi_sig[i],
                                                 chifit = chifit,
                                                 na.action = "na.omit",
                                                 algorithm = algorithm,
                                                 barchart_ylim = 650,
...
names(abSNF_barcharts_sig) = voi_sig
rm(loc, chifit)

# Multiplot (PNG) - bar charts
ggarrange(abSNF_barcharts_sig[[1]], abSNF_barcharts_sig[[2]], abSNF_barcharts_sig[[3]],
          abSNF_barcharts_sig[[4]], 
...
# Sunburst plot ###
library(plotly)
Pheno_sunburst_abSNF = clust_annot_pheno
Pheno_sunburst_abSNF$`ER status` = gsub("Unknown", "Unkn ER status", Pheno_sunburst_abSNF$`ER status`)
Pheno_sunburst_abSNF$`ER status` = gsub("Positive", "ER+", Pheno_sunburst_abSNF$`ER status`)
Pheno_sunburst_abSNF$`ER status` = gsub("Negative", "ER-", Pheno_sunburst_abSNF$`ER status`)
Pheno_sunburst_abSNF = Pheno_sunburst_abSNF %>%
  dplyr::select(abSNF, `ER status`) %>%
  group_by(abSNF, `ER status`) %>%
  summarise(Counts = n()) %>%
  as.data.frame()

sunburst_coloring_abSNF = data.frame(stringsAsFactors = FALSE,
                                   colors = tolower(gplots::col2hex(c("#2EC4B6", "#E71D36", 
                                                                      "#C11D9C", "#0F1682",  "grey40"))),
                                   labels = c("abSNF1", "abSNF2",
                                              "ER-", "ER+", "Unkn ER status"))

sunburstDF_abSNF = as.sunburstDF(Pheno_sunburst_abSNF, value_column = "Counts", add_root = FALSE) %>%
  inner_join(sunburst_coloring_abSNF, by = "labels")

pie_abSNF = plot_ly() %>%
  add_trace(ids = sunburstDF_abSNF$ids, labels= sunburstDF_abSNF$labels, 
            parents = sunburstDF_abSNF$parents, 
            values= sunburstDF_abSNF$values, type='sunburst', branchvalues = 'total',
            insidetextorientation='radial', maxdepth = 5,
            marker = list(colors = sunburstDF_abSNF$colors)) %>%
  layout(
    grid = list(columns =1, rows = 1),
    margin = list(l = 0, r = 0, b = 0, t = 0)
  )
pie_abSNF
rm(Pheno_sunburst_abSNF, sunburstDF_abSNF, sunburst_coloring_abSNF, pie_abSNF); gc()
...
for (i in 1:length(list_aff_S)) {
  ...
  nodes_data <- data.frame(name = V(g)$name) %>%
    inner_join(clust_annot_pheno %>% dplyr::select(samID, abSNF),
               by = c("name" = "samID"))
  
  # Set abSNF as a factor for coloring
  nodes_data[[algorithm]] <- as.factor(nodes_data[[algorithm]])
  V(g)$abSNF <- nodes_data[[algorithm]] # modify `$abSNF` manually
  
  # Set color based on abSNF
  V(g)$color <- fifelse(V(g)$abSNF == paste0(algorithm, "1"), "#2EC4B6", "#E71D36")
...
nodes_data <- data.frame(name = V(g)$name) %>%
  inner_join(clust_annot_pheno %>% dplyr::select(samID, abSNF), by = c("name" = "samID"))
nodes_data[[algorithm]] <- as.factor(nodes_data[[algorithm]])
V(g)$abSNF <- nodes_data[[algorithm]]
...
# Compare these abSNF results with the abSNF output from MOVICS ###
load("Results/MOVICS_baseline/MOVICS_TCGA_RNAseq-CNV-Methylation-miRNA-SNPs_eval_on_transNEO_moic.res.list.rda")
MOVICS_abSNF = moic.res.list$abSNF$clust.res

ARI_to_MOVICS_abSNF = calculate_ari_index(cluster_df1 = MOVICS_abSNF %>%
...
NMI_to_MOVICS_abSNF = calculate_nmi_index(cluster_df1 = MOVICS_abSNF %>%
                                          dplyr::rename(Sample.ID = samID,
                                                        Cluster = clust),
                                        cluster_df2 = abSNF_clusters,
                                        sample_col = "Sample.ID",
...
params = list(algorithm = algorithm, data_source = data_source, data_types = data_types,
              ...
              NMI_to_MOVICS_abSNF = NMI_to_MOVICS_abSNF, ARI_to_MOVICS_abSNF = ARI_to_MOVICS_abSNF,
