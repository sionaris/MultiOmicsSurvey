...
#SBATCH --nodes=1
#SBATCH --job-name=IntNMF_HPC_run
#SBATCH --output=logs/IntNMF_HPC_run.out
#SBATCH --error=logs/IntNMF_HPC_run.err
...
Rscript -e "packages <- c('parallel','doParallel','foreach','mclust', 'IntNMF'); install.packages(setdiff(packages, installed.packages()[,'Package']), repos='https://cran.r-project.org')"
