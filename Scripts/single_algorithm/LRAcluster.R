...
# However LRAcluster prefers features in columns so we transpose the matrices.
...
Fusions <- foreach(nn = num_neighbors_range, .combine = 'c', .packages = 'LRAclustertool') %:%
  foreach(sigma = sigma_range, .combine = 'c') %dopar% {
    ...
    fusion_result <- LRAcluster(iter_matrices, K = nn, t = n_iterations, parallel = FALSE)
    list(fusion_result)
  }
