...
SNF_clusters = as.data.frame(list(Sample.ID = optimal_SNF$Results$Sample.ID,
                                  Cluster = optimal_SNF$Results$Cluster))
...
for (i in seq_along(list_aff_S)) {
  ...
  nodes_data <- data.frame(name = V(g)$name) %>%
    inner_join(clust_annot_pheno %>% select(samID, SNF),
               by = c("name" = "samID"))
