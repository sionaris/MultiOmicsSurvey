...
if (!dir.exists(paste0(home, "/Results/Consensus_more_than_2/", 
                       algorithm))) {
  dir.create(paste0(home, "/Results/Consensus_more_than_2/", 
                    algorithm))
}
...
# Import clusterings from algorithms with MORE THAN 2 CLUSTERS IN THEIR OUTPUT
R_algorithms = c("iClusterBayes", "LRAcluster", "MFA", "wMKL")
Python_algorithms = c("MONET", "MSNE", "MOFA")
...
similarity_network_methods = c("MSNE")
multiple_kernel_learning = c("wMKL")
matrix_factorization_latent_variables = c("MFA", "MOFA", "LRAcluster")
graph_methods = c("MONET")
bayesian = c("iClusterBayes")

# Consensus Clustering #####
library(cluster)
sample_ids <- clusterings[[1]]$Sample.ID
N <- length(sample_ids)

# build the N×7 label matrix
lab_mat <- sapply(clusterings, function(df) {
  df$Cluster[ match(sample_ids, df$Sample.ID) ]
})
rownames(lab_mat) <- sample_ids

# Initialize accumulators
B             <- 100    # number of resampling iterations (tune as you like)
pItem         <- 0.8    # fraction of samples per iteration
coassoc_sum   <- matrix(0, N, N,
                        dimnames = list(sample_ids, sample_ids))
present_count <- matrix(0, N, N,
                        dimnames = list(sample_ids, sample_ids))

# Resampling loop
for(b in seq_len(B)) {
  # Subsample 80% of samples
  subsamp   <- sample(seq_len(N), size = floor(pItem * N))
  ids       <- sample_ids[subsamp]
  sub_lab   <- lab_mat[ids, , drop = FALSE]
  
  # Compute co-association on this subset
  m   <- nrow(sub_lab)
  ca  <- matrix(0, m, m)
  for(i in 1:m) for(j in i:m) {
    p       <- mean(sub_lab[i, ] == sub_lab[j, ])
    ca[i,j] <- ca[j,i]   <- p
  }
  
  # Accumulate
  coassoc_sum[ids, ids]   <- coassoc_sum[ids, ids] + ca
  present_count[ids, ids] <- present_count[ids, ids] + 1
}

# Final consensus matrix
consensus_mat <- coassoc_sum / present_count
diag(consensus_mat) <- 1
diss <- as.dist(1 - consensus_mat)
hc   <- hclust(diss, method = "average")

# Choose the best K from 2:10 via average silhouette
library(cluster)

sil_scores <- sapply(2:10, function(k) {
  labs_k <- cutree(hc, k = k)
  sil   <- silhouette(labs_k, diss)
  mean(sil[, "sil_width"])
})
...
getSilhouette_ggplot(sil      = silhouette,
                     fig.path = paste0(home, "/Results/Consensus_more_than_2/", algorithm),
                     fig.name = "Silhouette",
                     height   = 5.5,
                     width    = 5.5,
                     axis_label_size = 12,
...
openxlsx::write.xlsx(clust, paste0(home, "/Results/Consensus_more_than_2/", algorithm, "/",
...
clin_comp = compClinvar_single_algorithm(algorithm_name = algorithm,
                                         ...
                                         res.path = paste0(home, "/Results/Consensus_more_than_2/", algorithm, "/"),
                                         output_pdf = TRUE,
...
dgea.marker.up <- runMarker_single_algorithm(algorithm_name = algorithm,
                                             ...
                                             dat.path      = paste0(home, "/Results/Consensus_more_than_2/", algorithm), # path of DEA files
                                             res.path      = paste0(home, "/Results/Consensus_more_than_2/", algorithm), # path to save marker files
...
GSEAfiles_up <- sort(dir(paste0(home, "/Results/Consensus_more_than_2/", algorithm), 
                         pattern = "unique_upexpr_pathway.txt$"))
GSEAfiles_down <- sort(dir(paste0(home, "/Results/Consensus_more_than_2/", algorithm), 
                           pattern = "unique_downexpr_pathway.txt$"))

unique_upexpr_pathways = list()
for (i in 1:length(gsea.up$gsea.list)) {
  unique_upexpr_pathways[[i]] = data.table::fread(paste0(paste0(home, "/Results/Consensus_more_than_2/", algorithm), 
                                                         "/", GSEAfiles_up[i]),
                                                  header = TRUE, sep = "\t")
}
...
for (i in 1:length(gsea.down$gsea.list)) {
  unique_downexpr_pathways[[i]] = data.table::fread(paste0(paste0(home, "/Results/Consensus_more_than_2/", algorithm), 
                                                           "/", GSEAfiles_down[i]),
                                                    header = TRUE, sep = "\t")
}
...
hclust_input_up = prepare_gsea_output_for_hclust(gsea_output = gsea.up_unique, 
                                                 ...
                                                 dea.method = "limma", 
                                                 mo.method = "",
                                                 dat.path = paste0(home, "/Results/Consensus_more_than_2/", algorithm), 
...
saveWorkbook(wb, file = paste0(home, "/Results/Consensus_more_than_2/", algorithm, "/", 
...
if (!dir.exists(paste0(home, "/Results/Consensus_more_than_2/", algorithm, "/Supplement"))) {
  dir.create(paste0(home, "/Results/Consensus_more_than_2/", algorithm, "/Supplement"))
}
...
# Draw a heatmap of the final consensus matrix ###
create_MO_heatmap(matrix = consensus_mat, algorithm = algorithm, 
                  need.diag.zero = FALSE,
                  clust_annot_pheno = clust_annot_pheno ,
                  afh_colnames = afh_colnames, 
                  colors = colors_heatmap,
                  annColors = annColors,
                  heatmap_title = "Final consensus matrix heatmap",
                  cluster_colors = cluster_colors_heatmap,
                  cluster_rows_flag = FALSE,
                  cluster_cols_flag = FALSE,
                  splits_flag = TRUE,
                  legend_title = "Coassociation",
                  output_file_name = paste0(home, "/Results/Consensus_more_than_2/", algorithm, 
                                            "/Supplement/Consensus_matrix_heatmap.png"))
...
openxlsx::write.xlsx(output, 
                     paste0(home, 
                            "/Results/Consensus_more_than_2/", algorithm, "/Supplement/Chisq_tests.xlsx"),
                     overwrite = TRUE)
...
for (i in 1:length(list_aff_S)) {
  ...
  nodes_data <- data.frame(name = V(g)$name) %>%
    inner_join(clust_annot_pheno %>% dplyr::select(samID, CC),
               by = c("name" = "samID"))
  
  # Set CC as a factor for coloring
...
rmarkdown::render(paste0(getwd(), "/Results/Consensus_more_than_2/", algorithm,
                  ...
                  params = params, 
                  output_file = paste0(home, "/Results/Consensus_more_than_2/", 
...
# Save environment
save.image(paste0(home, "/Results/Consensus_more_than_2/CC_more_than_2.RData"))
