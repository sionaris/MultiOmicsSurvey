...
dist2.cimlr_mod2 = function (x, c = NA, name = NULL, method = "sqeuclidean") 
{
  ...
  if (method == "sqeuclidean") {
    ...
    dist = sqrt(t(rep(1, n2) %*% t(apply(t(x^2), MARGIN = 2, FUN = sum))) + 
                  (rep(1, n1) %*% t(apply(t(c^2), MARGIN = 2, FUN = sum))) - 
                  2 * (x %*% t(c)))
  } else if (method %in% c("maximum", "manhattan", "canberra", "binary", "minkowski")) {
    dist = as.matrix(dist(as.matrix(x), as.matrix(c), method = method))
  } else {
...
dist2.cimlr.weight_mod = function (x, weight, method = "sqeuclidean") 
{
  library(wMKL)
  y = t(t(x) * sqrt(weight))
  dist = dist2.cimlr_mod2(y, method = method)
  return(dist)
}
...
multiple.kernel.cimlr.weight_mod = function (x, cores.ratio = 0, weight, method = "sqeuclidean") 
{
  ...
  KK = 0
  sigma = seq(2, 1, -0.25)
  Diff = (dist2.cimlr.weight_mod(x, weight, method = method))
  Diff_sort = t(apply(Diff, MARGIN = 2, FUN = sort))
  m = dim(Diff)[1]
  n = dim(Diff)[2]
  allk = seq(10, 30, 2)
  ...
  D_Kernels = unlist(parLapply(cl, 1:length(allk), fun = function(l, 
    ...
    if (allk_fun[l] < (nrow(x_fun) - 1)) {
      ...
      for (j in 1:length(sigma_fun)) {
        W = dnorm(Diff_fun, 0, sigma_fun[j] * Sig)
        D_Kernels[[KK_fun + l + j]] = Matrix::Matrix((W + t(W))/2, 
                                             sparse = TRUE, doDiag = FALSE)
      }
      return(D_Kernels)
    }
  ...
  for (i in 1:length(D_Kernels)) {
    K = D_Kernels[[i]]
    k = 1/sqrt(Matrix::diag(K) + 1)
    G = K * (k %*% t(k))
    G1 = apply(array(0, c(length(Matrix::diag(G)), length(Matrix::diag(G)))), 
               MARGIN = 2, FUN = function(x) {
                 x = Matrix::diag(G)
               })
    G2 = t(G1)
    D_Kernels_tmp = (G1 + G2 - 2 * G)/2
    D_Kernels_tmp = D_Kernels_tmp - Matrix::diag(Matrix::diag(D_Kernels_tmp))
    D_Kernels[[i]] = Matrix::Matrix(D_Kernels_tmp, sparse = TRUE, 
                            doDiag = FALSE)
  }
  return(D_Kernels)
}
                                                         ...
                                                         methods = rep("sqeuclidean", length(all_data))) 
{
  ...
  for (data_types in 1:length(all_data)) {
    ...
    curr_method = methods[data_types]
    if (data_types == 1) {
      D_Kernels = multiple.kernel.cimlr.weight_mod(t(curr_X), 
                                               cores.ratio, curr_weight,
                                               method = curr_method)
      distX = array(0, c(dim(D_Kernels[[1]])[1], dim(D_Kernels[[1]])[2]))
      for (i in 1:length(D_Kernels)) {
        distX = distX + D_Kernels[[i]]
      ...
      W = wMKL:::network.diffusion.numc(W, max(ceiling(ncol(X)/20), 
                                        10))
    }
    else {
      D_Kernels = c(D_Kernels, multiple.kernel.cimlr.weight_mod(t(curr_X), 
                                                            cores.ratio, curr_weight,
                                                            method = curr_method))
      distX = array(0, c(dim(D_Kernels[[1]])[1], dim(D_Kernels[[1]])[2]))
      for (i in 1:length(D_Kernels)) {
      ...
      W = W + wMKL:::network.diffusion.numc(W0, max(ceiling(ncol(X)/20), 
                                             10))
    }
  }
  Quality = wMKL:::Estimate_Number_of_Clusters_given_graph(W, NUMC)
  Quality_plus = wMKL:::Estimate_Number_of_Clusters_given_graph(W, 
                                                         NUMC + 1)
  Quality_minus = wMKL:::Estimate_Number_of_Clusters_given_graph(W, 
                                                          NUMC - 1)
                             ...
                             methods = rep("sqeuclidean", length(X))) 
{
  ...
  S0 = max(max(distX)) - distX
  cat("Performing network diffusion.\n")
  S0 = wMKL:::network.diffusion(S0, k)
  S0 = wMKL:::dn.cimlr(S0, "ave")
  S = (S0 + t(S0))/2
  D0 = Matrix::diag(apply(S, MARGIN = 2, FUN = sum))
  L0 = D0 - S
  eig1_res = wMKL:::eig1(L0, c, 0)
  F_eig1 = eig1_res$eigvec
  temp_eig1 = eig1_res$eigval
  evs_eig1 = eig1_res$eigval_full
  F_eig1 = wMKL:::dn.cimlr(F_eig1, "ave")
  converge = vector()
  for (iter in 1:NITER) {
    cat("Iteration: ", iter, "\n")
    distf = wMKL:::L2_distance_1(t(F_eig1), t(F_eig1))
    A = array(0, c(num, num))
    b = idx[, 2:dim(idx)[2]]
    a = apply(array(0, c(num, ncol(b))), MARGIN = 2, FUN = function(x) {
      x = 1:num
    ...
    A[is.nan(A)] = 0
    S = (1 - beta) * A + beta * S
    S = wMKL:::network.diffusion(S, k)
    S = (S + t(S))/2
    D = Matrix::diag(apply(S, MARGIN = 2, FUN = sum))
    L = D - S
    F_old = F_eig1
    eig1_res = wMKL:::eig1(L, c, 0)
    F_eig1 = eig1_res$eigvec
    temp_eig1 = eig1_res$eigval
    ev_eig1 = eig1_res$eigval_full
    F_eig1 = wMKL:::dn.cimlr(F_eig1, "ave")
  ...
  if (length(no.dim) == 1) {
    U_index = seq(ncol(U), (ncol(U) - no.dim + 1))
    F_last = wMKL:::tsne(S, k = no.dim, initial_config = U[, U_index])
  }
  else {
    F_last = list()
    for (i in 1:length(no.dim)) {
      U_index = seq(ncol(U), (ncol(U) - no.dim[i] + 1))
      F_last[i] = list(wMKL:::tsne(S, k = no.dim[i], initial_config = U[, 
                                                                 U_index]))
    }
  }
  execution.time = proc.time() - ptm
  ...
  y = kmeans(F_last, c, nstart = 200)
  ydata = wMKL:::tsne(S)
  y_spectral = wMKL:::spectralClustering(S, c)
  results = list()
  results[["y"]] = y
...
TCGAanalyze_survival_custom3 <- function(
  ...
  ## 1  housekeeping --------------------------------------------------------
  stopifnot(clusterCol %in% names(data))
  needed <- c("vital_status","days_to_death","days_to_last_followup")
  if (!all(needed %in% names(data)))
    stop("Missing columns: ", paste(setdiff(needed, names(data)), collapse = ", "))
  if (!is.null(adjustVars) && !all(adjustVars %in% names(data)))
    stop("Some adjustVars not found in data")
  
  data$days_to_death[is.na(data$days_to_death)] <-
    data$days_to_last_followup[is.na(data$days_to_death)]
  data$s    <- grepl("dead|deceased", data$vital_status, ignore.case = TRUE)
  data$type <- factor(data[[clusterCol]])
  
  if (is.null(color))
    color <- viridisLite::viridis(nlevels(data$type))
  
  miss_pct <- 100 * mean(!stats::complete.cases(data[, c(clusterCol, adjustVars)]))
  x_ann <- if (!is.null(xlim)) min(xlim) else 0
  y_ann <- 0.05
  
  
  ## 2  Kaplan–Meier branch -------------------------------------------------
  if (is.null(adjustVars)) {
    
    fitKM <- survival::survfit(survival::Surv(days_to_death, s) ~ type, data = data)
    lr    <- survival::survdiff(survival::Surv(days_to_death, s) ~ type, data = data)
    p_lr  <- stats::pchisq(lr$chisq, df = length(lr$n) - 1, lower.tail = FALSE)
    
    if (is.null(labels)) {
      tab <- table(data$type)
      labels <- paste0(names(tab), " (n=", as.integer(tab), ")")
    }
    ...
    if (pvalue)
      ...
      ggplot2::annotate("text", x = x_ann, y = y_ann,
                        hjust = 0, label = sprintf("Log-rank p = %.3g", p_lr),
                        size = 5)
    
    final_plot  <- survObj$plot +
      ggplot2::labs(title = main, x = xlab, y = ylab, colour = legend)
    final_table <- survObj$table
    diag_p      <- p_lr
    
    sep_ok <- ph_ok <- vif_ok <- lin_ok <- NA
    ...
    fml    <- stats::as.formula(
      paste("Surv(days_to_death, s) ~", paste(c("type", adjustVars), collapse = " + "))
    )
    coxObj <- survival::coxph(fml, data = data)
    
    ## 3a  diagnostics
    sep_ok <- all(is.finite(stats::coef(coxObj)))
    
    zph    <- tryCatch(survival::cox.zph(coxObj), error = function(e) NULL)
    ph_ok  <- !is.null(zph) && all(zph$table[, "p"] > ph_threshold)
    
    vif_ok <- NA
    vif_vals <- tryCatch({
      if (requireNamespace("car", quietly = TRUE))
        suppressWarnings(car::vif(coxObj))
    }, error = function(e) NA_real_)
    if (!all(is.na(vif_vals)))
      vif_ok <- all(vif_vals < vif_cutoff, na.rm = TRUE)
    
    ## linearity check – silence “ties” note
    lin_ok <- TRUE
    numeric_adj <- adjustVars[sapply(data[adjustVars], is.numeric)]
    if (length(numeric_adj)) {
      mf   <- model.frame(coxObj)          # rows actually used
      mart <- residuals(coxObj, type = "martingale")
      lin_p <- vapply(numeric_adj, function(v) {
        z <- suppressWarnings(
          stats::cor.test(mart, mf[[v]], method = "spearman", exact = FALSE)
        )
        z$p.value
      }, numeric(1))
      lin_ok <- all(lin_p > linearity_alpha)
    }
    
    ## 3b  p-value (LRT, fall back to Wald if NA)
    lrt   <- drop1(coxObj, scope = ~ . - type, test = "Chisq")
    p_cox <- lrt["type", "Pr(>Chi)"]
    if (is.na(p_cox)) {
      coefs <- summary(coxObj)$coefficients
      waldP <- coefs[grep("^type", rownames(coefs)), "Pr(>|z|)"]
      p_cox <- max(waldP, na.rm = TRUE)
    }
    
    final_plot <- survminer::ggadjustedcurves(
      fit = coxObj, variable = "type", data = data,
      method = "conditional", conf.int = conf.int,
      legend.title = legend, xlim = xlim, color = color
    ) +
      ggplot2::annotate("text", x = x_ann, y = y_ann,
                        hjust = 0, label = sprintf("LRT p = %.3g", p_cox),
                        size = 5) +
      ggplot2::labs(title = main, x = xlab, y = ylab, colour = legend)
  ...
  ## 4  save figure ---------------------------------------------------------
  dir.create(dirname(save.filename), recursive = TRUE, showWarnings = FALSE)
  if (risk.table && !is.null(final_table)) {
    g <- gridExtra::arrangeGrob(final_plot, final_table,
                                heights = c(1 - risk.table.height, risk.table.height))
    ggsave(g, filename = save.filename, width = save.width, height = save.height, dpi = save.dpi)
  } else {
    ggsave(final_plot, filename = save.filename, width = save.width, height = save.height, dpi = save.dpi)
  }
  
  
  ## 5  dashboard line ------------------------------------------------------
  if (!is.null(summary.filename)) {
    dir.create(dirname(summary.filename), recursive = TRUE, showWarnings = FALSE)
    cat(sprintf("%s\tp=%.3g\tPH:%s\tVIF:%s\tLin:%s\tSep:%s\tMiss:%.1f%%\n",
                basename(save.filename), diag_p,
                ifelse(is.na(ph_ok),  "n/a", ifelse(ph_ok,  "OK", "FAIL")),
                ifelse(is.na(vif_ok), "n/a", ifelse(vif_ok, "OK", "FAIL")),
                ifelse(is.na(lin_ok), "n/a", ifelse(lin_ok, "OK", "WARN")),
                ifelse(is.na(sep_ok), "n/a", ifelse(sep_ok,"OK", "FAIL")),
                miss_pct),
        file = summary.filename, append = TRUE)
  }
  ...
  invisible(list(
    ...
    pvalue      = diag_p,
    diagnostics = list(
      ph_ok = ph_ok, vif_ok = vif_ok, lin_ok = lin_ok,
      sep_ok = sep_ok, missing_pct = miss_pct
    )
  ))
}